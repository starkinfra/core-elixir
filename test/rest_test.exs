...
defmodule StarkCoreTestRest.GetPage do
  ...
  test "Should get balance using get_page" do
    {:ok, {_, response}} = Rest.get_page(:bank, Balance.resource(),[])
  ...
  test "Should fail silently using get_page on non existent resource" do
    assert {:error, _} = Rest.get_page(:bank, Balance.non_existent_resource(),[])
  end

  @tag :get_page
  test "Should fail and raise error using get_page! on non existent resource" do
    assert_raise RuntimeError, fn ->
      Rest.get_page!(:bank, Balance.non_existent_resource(),[])
    end
  end
end
...
defmodule StarkCoreTestRest.GetList do
  ...
  test "Should get a list of invoices using get_list" do
    [ok: invoice] = Rest.get_list(:bank, Invoice.resource(),[limit: 3])
      |> Enum.take(1)
  ...
  test "Should silently fail to get a list of invoices using get_list" do
    assert [error: _invoice] = Rest.get_list(:bank, Balance.non_existent_resource(),[limit: 3])
      |> Enum.take(1)
  end

  @tag :get_list
  test "Should get a list of invoices using get_list!" do
    [invoice] = Rest.get_list!(:bank, Invoice.resource(),[limit: 3])
      |> Enum.take(1)
  ...
  test "Should fail and raise error using get_list!" do
    assert_raise RuntimeError, fn ->
      Rest.get_list!(:bank, Balance.non_existent_resource(),[limit: 3])
        |> Enum.take(1)
    end
  end
end
...
defmodule StarkCoreTestRest.GetId do
  ...
  test "Should get an invoice by id using get_id" do
    # list some invoice
    [ok: invoice_listed] = Rest.get_list(:bank, Invoice.resource(),[limit: 3])
      |> Enum.take(1)

    # Get by ID
    {:ok, invoice} = Rest.get_id(:bank, Invoice.resource(), invoice_listed.id, [])
  ...
  @tag :get_id
  test "Should get an invoice by id using get_id!" do
    [invoice_listed] = Rest.get_list!(:bank, Invoice.resource(),[limit: 3])
      |> Enum.take(1)

    invoice = Rest.get_id!(:bank, Invoice.resource(), invoice_listed.id, [])
  ...
  test "Should silently fail using get_id on non existant id" do
    assert {:error, _} = Rest.get_id(:bank, Invoice.resource(), "123412341234", [])
  end

  @tag :get_id
  test "Should raise error using get_id! on non existant id" do
    assert_raise RuntimeError, fn ->
      Rest.get_id!(:bank, Invoice.resource(), "123412341234", [])
    end
  end
end
...
defmodule StarkCoreTestRest.Rest do
  ...
  test "Should get qrcode of invoice using get_content" do
    ...
    assert {:ok, _response} = Rest.get_content(:bank, Invoice.resource(), invoice_listed.id, "pdf", [], nil)
  end
  ...
  test "Should get qrcode of invoice using get_content!" do
    ...
    response = Rest.get_content!(:bank, Invoice.resource(), invoice_listed.id, "pdf", [], nil)
  ...
  test "Should silently fail to get qrcode of invoice using get_content on non existant Invoice ID" do
    assert {:error, _response} = Rest.get_content(:bank, Invoice.resource(), "123412341234", "pdf", [], nil)
  end

  @tag :get_content
  test "Should raise error to get qrcode of invoice using get_content on non existant Invoice ID" do
    assert_raise RuntimeError, fn ->
      Rest.get_content!(:bank, Invoice.resource(), "123412341234", "pdf", [], nil)
    end
  end
end
...
defmodule StarkCoreTestRest.Post do
  ...
  test "Should create a new Invoice using post" do
    ...
    assert {:ok, _} = Rest.post(:bank, Invoice.resource(), invoices, [])
  end
  ...
  test "Should create a new Invoice using post!" do
    ...
    invoices_created = Rest.post!(:bank, Invoice.resource(), invoices, [])
  ...
  test "Should silently fail to create an invoice using post" do
    ...
    assert {:error, _} = Rest.post(:bank, Invoice.resource(), invoices, [])
  end
  ...
  test "Should raise error to create an invoice using post!" do
    ...
    assert_raise RuntimeError, fn ->
      Rest.post!(:bank, Invoice.resource(), invoices, [])
    end
  end
end
...
defmodule StarkCoreTestRest.PostSingle do
  ...
  setup do
    ...
    Enum.map(webhook_stream, fn item ->
      deletion_response = Rest.delete_id(:bank, Webhook.resource(), item.id, [])
...
defmodule StarkCoreTestRest.Delete do
  ...
  setup do
    ...
    {:ok, created_boleto} = Rest.post(:bank, Boleto.resource(), boletos_to_create, [])
  ...
  test "Should delete the created Boleto by its ID", state do
    assert {:ok, _deleted_response} = Rest.delete_id(:bank, Boleto.resource(), state[:boleto].id, [])
  end

  @tag :delete
  test "Should delete the created Boleto by its ID using delete_id!", state do
    assert Rest.delete_id!(:bank, Boleto.resource(), state[:boleto].id, [])
  end

  @tag :delete_fail
  test "Should silently fail trying delete a Boleto using delete_id with an INVALID_ID" do
    assert {:error, _deleted_response} = Rest.delete_id(:bank, Boleto.resource(), "INVALID_ID", [])
  end

  @tag :delete_fail
  test "Should raise error when trying delete a Boleto using delete_id! with an INVALID_ID" do
    assert_raise RuntimeError, fn ->
      Rest.delete_id!(:bank, Boleto.resource(), "INVALID_ID", [])
    end
  end
...
defmodule StarkCoreTestRest.PatchId do
  ...
  setup do
    ...
    {:ok, created_invoice} = Rest.post(:bank, Invoice.resource(), invoices_to_create, [])
  ...
  test "Should patch Invoice by its ID", state do
    assert {:ok, _response} = Rest.patch_id(:bank, Invoice.resource(), state[:invoice].id,
    %{
      amount: 20000,
      expiration: 3600
    })
  ...
  test "Should patch Invoice by its ID using patch_id!", state do
    assert Rest.patch_id(:bank, Invoice.resource(), state[:invoice].id,
    %{
      amount: 20000,
      expiration: 3600
    })
  ...
  test "Should silently fail patch Invoice using patch_id with INVALID_ID" do
    assert {:error, _response} = Rest.patch_id(:bank, Invoice.resource(), "INVALID_ID",
    %{
      amount: 20000,
      expiration: 3600
    })
  ...
  test "Should raise error trying to patch Invoice using patch_id! with INVALID_ID" do
    assert_raise RuntimeError, fn ->
      Rest.patch_id!(:bank, Invoice.resource(), "INVALID_ID",
        %{
          amount: 20000,
          expiration: 3600
        }
...
defmodule StarkCoreTestRest.GetSubResource do
  ...
  setup do
    [ok: paid_invoice] = Rest.get_list(:bank, Invoice.resource(),
      status: "paid",
      limit: 1)
      |> Enum.take(1)
