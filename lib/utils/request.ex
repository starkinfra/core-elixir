defmodule StarkCore.Utils.Request do
  ...
  @spec fetch(atom(), atom(), String.t(), [payload: map(), query: list(), version: String.t(), user: Project.t() | Organization.t()]) :: {:ok, String.t()} | {:error, [Error.t()]}
  def fetch(service, method, path, options \\ []) do
    ...
    request(
      user,
      method,
      URL.get_url(service, user.environment, version, path, query),
      payload
    ) |> process_response
  end
